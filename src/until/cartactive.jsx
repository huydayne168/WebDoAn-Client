...
let activeVoucher = EMPTY_VOUCHER;

export function SetActiveVoucher(voucher = EMPTY_VOUCHER) {
  activeVoucher = voucher || EMPTY_VOUCHER;
}

export function LoadData() {
  var list = JSON.parse(localStorage.getItem("cart")) || [];
  var voucher = activeVoucher;
  var str = "";
  var discount = 0;
  var tamtinh = 0;
  for (let x of list) {
  ...
  var minimumOrder = Number(voucher.value) || 0;
  var discountAmount = Number(voucher.discount_amount) || 0;

  if (voucher.coupon_name && voucher.coupon_name !== "novoucher" && tamtinh >= minimumOrder) {
    if (voucher.discount_type === "percent") {
      discount = Math.floor((tamtinh * discountAmount) / 100);
    } else {
