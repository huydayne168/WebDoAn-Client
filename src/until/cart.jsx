...
function AddProduct() {
  ...
  useEffect(() => {
    ...
    function addToCart(item) {
      var list;
      if (item.stockLimit && item.stock <= 0) {
          alert("Sản phẩm đã hết hàng.");
          return false;
      }
      if (item.stockLimit && item.quantity > item.stock) {
      ...
      if (localStorage.getItem("cart") === null) {
          ...
          for (let x of list) {
              if (x.id === item.id) {
                  ...
                  if (item.stockLimit && x.quantity + item.quantity > item.stock) {
                    alert(`Chỉ còn ${item.stock} sản phẩm trong kho.`);
                    return false;
                  }
    ...
    $('.btn-addCart').click(function() {
      if ($(this).text() === "Thêm vào giỏ hàng") {
          ...
          var quantity = parseInt($(".quantity-input").val() || $(".quantity span").text(), 10) || 1;
          var stockValue = $(this).attr("data-stock") || $(".quantity").attr("data-stock");
          var stockLimit = stockValue !== undefined;
          var stock = parseInt(stockValue, 10) || 0;

          if (stockLimit && stock <= 0) {
              alert("Sản phẩm đã hết hàng.");
              return;
          }

          if (stockLimit && quantity > stock) {
